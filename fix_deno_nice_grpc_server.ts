...
const benchmarkService = (proto as Record<string, { BenchmarkService: Record<string, unknown> }>).BenchmarkService;

async function main() {
  const server = createServer();

  // Filter definition to only include actual method descriptors (those with a 'path' property)
  const filteredDef: Record<string, unknown> = {};
  for (const [key, value] of Object.entries(benchmarkService as Record<string, unknown>)) {
