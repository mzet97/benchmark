...
class DatabaseService {
  ...
  async getUser(id: number): Promise<any> {
    try {
      ...
      // {Id, Name, Email, CreatedAt, IsActive} or {id, email, name, age,
      // created_at} -- neither matched the contract.
  ...
  async getComplexQuery(days: number): Promise<any[]> {
    try {
      ...
      // without a tiebreak, so equal values came back in arbitrary order.
      const result = await this.client.query(`
        SELECT
          u.id AS "userId",
      ...
      // Rethrow. This used to `return []`, which the handler turns into a 200
      ...
      // invariante 8 in docs/ACTION_PLAN.md.
