# Spring Boot Native on GraalVM

High-performance REST API benchmark using Spring Boot compiled as a GraalVM native image for instant startup and low memory footprint.

## Tech Stack

- Java 21 (GraalVM Community Edition)
- Spring Boot 3.2.0 (native image)
- Spring Data JPA + PostgreSQL
- Spring Data Redis
- Maven build system with native-image plugin
...
```bash
docker build -t benchmark-graalvm-gspring .
docker run -p 3000:3000 \
    -e DATABASE_URL="jdbc:postgresql://host:5432/benchmark_api" \
    -e REDIS_URL="redis://host:6379" \
    benchmark-graalvm-gspring
```
