# Deno.serve - Benchmark API

REST API benchmark implementation using native Deno.serve() (no framework).

## Tech Stack

- **Runtime**: Deno 2.0 (native HTTP server)
- **Framework**: None (Deno.serve())
...
# Docker
./build.sh docker
docker run -p 3000:3000 benchmark/deno-serve:latest
