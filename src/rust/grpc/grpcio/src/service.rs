...
impl BenchmarkGrpc for BenchmarkService {
    ...
    fn get_json_items(
        ...
        let count = crate::canonical::item_count(req.limit);
        ...
        let items: Vec<JsonItem> = (0..count)
            .map(|i| {
                ...
                item.id = i;
                item.uuid = crate::canonical::uuid(i);
                item.name = crate::canonical::name(i);
                item.email = crate::canonical::email(i);
                item.created_at = crate::canonical::CANONICAL_CREATED_AT.to_string();
                item.is_active = crate::canonical::is_active(i);
                item
            })
            .collect();
        ...
        let f = async move {
            ...
            let data: Vec<UserOrderStats> = rows
                ...
                .map(|row| {
                    ...
                    stats.total_orders = row.get::<_, i64>(2) as i32;
