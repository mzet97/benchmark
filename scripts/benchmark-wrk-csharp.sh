...
echo "=========================================="
echo "C# Minimal API (.NET 9) - WRK Benchmark Suite"
echo "=========================================="
echo ""

# Configuration
SERVICE="${SERVICE:-csharp-minimalapi}"
...
if kubectl get svc ${SERVICE} -n ${NAMESPACE} &>/dev/null; then
    echo "✓ Detected Kubernetes service: ${SERVICE} in namespace ${NAMESPACE}"
    URL=${BASE_URL}
else
...
echo "  Results show request throughput and latency percentiles"
echo "  Compare results across different language implementations"
echo ""

echo "To view detailed results:"
echo "  wrk outputs are displayed above"
echo "  Save results to file: ./benchmark-wrk-csharp.sh > results.txt"
echo ""

# Additional metrics
echo "Additional Metrics to Collect:"
echo "  1. CPU Usage: kubectl top pods -l app=${SERVICE} -n ${NAMESPACE}"
echo "  2. Memory Usage: kubectl top pods -l app=${SERVICE} -n ${NAMESPACE}"
